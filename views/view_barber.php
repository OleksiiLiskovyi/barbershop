...
if (isset($_GET['delete_comment_id'])) {
    $del_id = (int)$_GET['delete_comment_id'];
    if ($isAdmin) {
    ...
    header("Location: index.php?action=view_barber&id=$id"); exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_comment']) && $current_user_id) {
    ...
    if (!empty($text)) {
        if ($c_id > 0) mysqli_query($link, "UPDATE comments SET comment_text = '$text' WHERE id = $c_id AND user_id = $current_user_id");
        else mysqli_query($link, "INSERT INTO comments (user_id, barber_id, comment_text) VALUES ($current_user_id, $id, '$text')");
        header("Location: index.php?action=view_barber&id=$id"); exit;
    }
}

$b = mysqli_fetch_assoc(mysqli_query($link, "SELECT u.first_name, u.last_name, b.image, b.bio, c.name as cat_name FROM barbers b JOIN users u ON b.user_id = u.id JOIN categories c ON b.category_id = c.id WHERE b.id = $id"));
...
$edit_id = (int)($_GET['edit_comment_id'] ?? 0); $edit_text = '';
if ($edit_id > 0 && $current_user_id) {
    ...
    if ($row_edit = mysqli_fetch_assoc($res_edit)) $edit_text = $row_edit['comment_text'];
}

if (!$b) die("<main class='content'><div class='form-card'><h2>Майстра не знайдено</h2></div></main>");
?>
